<template>
	<div class="realUpVideo">
		...
		<div class="upVideoContent">
			<form action="">
				<label>
					<input type="radio" name="videoOrMany" value="video" checked>视频
				</label>
				<label>
					<input type="radio" name="videoOrMany" value="many">专辑
				</label>
				<p @click="showDialog">
					<span>+为视频添加文档</span>
				</p>
				<div class="upFormDiv">
					...
					<div>
						<p class="tagTitle">请为你的视频填写技术标签,使用空格隔开,不超过10个</p>
						<span>技术标签：</span>
						<input type="text" class="tagInput" @keydown="createTag($event)" :value=tagInput >
						<div class="tags">
							<p class="tag" v-for="tag in tags">
								<span :flag=tag.tagId>{{tag.tagName}}</span>
								<span @click="cancelTag($event)"><i class="fa fa-close"></i></span>
							</p>
						</div>
					</div>
					<div>
...
<script>
	...
	export default {
		...
		methods:{
			...
			turnUpVideo(){
				this.$router.push({path: '/personal/upVideo',query:{msg:'upVideo'}})
			},
			//点击加载视频l类型
			loadVideoTypes(){
				//下拉菜单进行隐藏和显示
